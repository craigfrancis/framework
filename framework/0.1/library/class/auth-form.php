<?php
	...
	class auth_form_base extends auth {
		...
		// Variables
			...
			protected $update_field_password_new_required = false; // Don't set directly, just call $auth->update_field_password_new_1_get(array('required' => true));
			...
			// Request
				...
				public function update_validate($a = NULL) {
					...
					if ($form === NULL) {
						...
						exit_with_error('Cannot call auth::update_validate() without first returning one or more update fields.');
		...
		// Support functions
			...
			protected function field_password_1_get($form, $config) { // Used in login, register, update (x2), reset.
			...
			protected function field_password_2_get($form, $config) { // Used in register, update, reset.
