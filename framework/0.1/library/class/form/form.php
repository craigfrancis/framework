					...
					if ($this->form_submitted && $this->csrf_error_html != NULL) { // Cant type check, as html() will convert NULL to string
						...
						if ($this->print_page_skipped === true && config::get('request.method') == 'GET' && $this->form_method == 'POST') {
							...
							if (in_array('fetch', $checks) && $this->fetch_allowed !== false) {
								...
								} else if ($fetch_values['dest'] === 'document' && $fetch_values['mode'] === 'same-origin' && $fetch_values['site'] === 'same-origin' && $fetch_values['user'] === '?1' && !in_array('cors', $this->fetch_allowed['mode'])) {
									$csrf_report = true; // At least one bot is doing this, the mode 'same-origin' suggests it's doing something like Fetch/XMLHttpRequest (i.e. not 'navigate'), but due to the last check, fetch_allowed_fetch() hasn't been used, so we are going to block.
									$csrf_block = true;
								} else if ($fetch_values['dest'] === 'document' && in_array($fetch_values['mode'], ['cors', 'same-origin']) && $fetch_values['site'] === NULL && $fetch_values['user'] === NULL && !in_array('cors', $this->fetch_allowed['mode'])) { // This last 'cors' condition skips this bot blocker when fetch_allowed_fetch().
