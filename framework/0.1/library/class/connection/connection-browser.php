...
//--------------------------------------------------
	...
	class connection_browser_base extends check {
		...
		// Setup
			...
			public function user_agent_set($user_agent) {
				...
				$this->connection->header_set('User-Agent', $user_agent);
			...
			public function encoding_accept_set($type, $decode = false) { // Currently supports gzip... maybe later deflate?
				...
				if ($type !== NULL) {
					$this->connection->header_set('Accept-Encoding', $type);
				}
		...
		//--------------------------------------------------
		// Expose connection - don't extend the connection
		// class as we don't want access to get/post,
		// value, cookies, etc methods

			public function request_full_get() {
				return $this->connection->request_full_get();
			}

			public function response_full_get() {
				return $this->connection->response_full_get();
			}

			public function response_code_get() {
				return $this->connection->response_code_get();
			}

			public function response_mime_get() {
				return $this->connection->response_mime_get();
			}

			public function response_headers_get() {
				return $this->connection->response_headers_get();
			}

			public function response_header_get($field) {
				return $this->connection->response_header_get($field);
			}

			public function response_header_get_all($field) {
				return $this->connection->response_header_get_all();
			}
		...
		// Support functions
			...
			private function _send($url, $data = '', $method = 'GET') {
				...
				// Get page
					...
					do {
						...
						// Cookies
							...
							$this->connection->cookies_raw_set($cookies);

						//--------------------------------------------------
						// Debug

							if ($this->debug) {
								debug(array('url' => $url, 'method' => $method, 'data' => $data, 'cookies' => $cookies, 'headers' => $this->connection->headers_get()));
							}
						...
						// Request
							...
							if ($method == 'GET' || $method == '') {
								...
								$this->connection->get($url, $data);

							} else if ($method == 'POST') {

								$this->connection->post($url, $data);
						...
						// Error
							...
							$connection_error = $this->connection->error_message_get();
							if ($connection_error !== NULL) {
								$this->error($connection_error, $this->connection->error_details_get());
							}
						...
						// Response
							...
							$this->current_url = $url;
							$this->current_data = $this->connection->response_data_get();
							$this->current_code = $this->connection->response_code_get();

							// if ($this->debug) {
							// 	debug($this->connection->response_full_get());
							// }
						...
						// Cookies
							...
							foreach ($this->connection->response_header_get_all('Set-Cookie') as $header_cookie) {
				...
				// Referrer
					...
					$this->connection->header_set('Referer', $this->current_url);
