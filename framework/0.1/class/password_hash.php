	...
	class password_hash_base extends check {
		...
		// Support functions
			...
			private function _crypt_private($password, $setting) {
				...
				if ($id != '$P$' && $id != '$H$') { // We use "$P$", phpBB3 uses "$H$" for the same thing
					return $output;
				}
...
// Testing
	...
	if (true) {
		...
		// Init
			...
			class password_hash extends password_hash_base {
			}
			...
			$unportable_hasher = new password_hash(8, false);
			$portable_hasher = new password_hash(8, true);
		...
		// Unportable
			...
			$hash = $unportable_hasher->hash($correct);

			echo 'Hash: ' . $hash . "\n";

			$check = $unportable_hasher->check($correct, $hash);
			if ($check) $ok++;
			echo "  Check correct: '" . $check . "' (should be '1')\n";

			$check = $unportable_hasher->check($wrong, $hash);
			if (!$check) $ok++;
		...
		// Portable check new
			...
			$hash = $portable_hasher->hash($correct);

			echo 'Hash: ' . $hash . "\n";

			$check = $portable_hasher->check($correct, $hash);
			if ($check) $ok++;
			echo "  Check correct: '" . $check . "' (should be '1')\n";

			$check = $portable_hasher->check($wrong, $hash);
			if (!$check) $ok++;
			echo "  Check wrong: '" . $check . "' (should be '0' or '')\n";
		...
		//--------------------------------------------------
		// Portable check existing

			$hash = '$P$9IQRaTwmfeRo7ud9Fh4E2PdI0S3r.L0'; // A portable hash for 'test12345'
