...
// String conversion
	...
	// Human to...
		...
		function human_to_ref($text) {
			...
			$text = preg_replace('/[^a-z0-9]+/', '_', $text); // TODO: Allow hyphens, so the radio field "-1" does not get changed to "1", and the URL "/a/sub-page/" gets the ID "#p_a_sub-page".
			$text = trim($text, '_');
...
// Other string/array functions
	...
	function split_words($text) {
		$words = [];
		$split = preg_split('/\s+/u', strval($text));
		if ($split === false) {
			return [];
		}
		foreach ($split as $word) { // Only on whitespace, so not "O'Brien"
...
// Check that an email address is valid
	...
	function is_email($email, $domain_check = true) {
		...
		$format_valid = preg_match('/^(?:[a-z0-9\.!#$%&\'\*\+\/=?^_`{|}~-]+|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(\w[-._\w]*\.[a-z]{2,})$/i', $email, $matches);
...
// Looks like spam
	...
	function is_spam_like($message) {
		...
		return preg_match('/(https?:\/\/|\s@\w|\b(SEO|BTC|bitcoin|cialis|viagra|marketing)\b)/i', $message); // The @ test allows emails, but not Twitter/Telegram/etc handles.
	}
